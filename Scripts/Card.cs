...
public class Card : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public void AtualizarUI()
    {
        ...
        if (nomeText != null) custoText.text = "🪙 " + custo;
        if (raridadeText != null) raridadeText.text = raridade.ToString();

        AtualizarCorPorRaridade();
    }

    public void AtualizarCorPorRaridade()
    {
        if (background == null) return;

        switch (raridade)
        {
            case CardRarity.Comum:
            ...
            case CardRarity.Epica:
                background.color = new Color(0.6f, 0f, 0.8f); // roxo
                break;
            case CardRarity.Lendaria:
                background.color = new Color(1f, 0.84f, 0f); // dourado
                break;
        }
    }
    ...
    public void OnPointerEnter(PointerEventData eventData)
    {
        ...
        if (cardPreviewPrefab != null && currentPreview == null)
        {
            ...
            if (previewCardComponent != null)
            {
                ...
                previewCardComponent.custo = this.custo;
